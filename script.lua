...
-- Получаем ссылку на необходимые сервисы и объекты
local Workspace = game:GetService("Workspace")
local Players = game:GetService("Players")
local RunService = game:GetService("RunService")

-- Получаем локального игрока и его leaderstats
local player = Players.LocalPlayer
local leaderstats = player:WaitForChild("leaderstats")
local sizeStat = leaderstats:WaitForChild("Size")  -- Учитываем регистр

-- Получаем папку Food в Workspace
local FoodFolder = Workspace:WaitForChild("Food")

-- Устанавливаем скорость перемещения
local moveSpeed = 10  -- Измените значение скорости по своему усмотрению

-- Функция для плавного перемещения игрока
local function moveToPosition(player, targetPosition)
    if player and player.Character then
        ...
        if humanoidRootPart then
            ...
            while distance > 1 do
                ...
                -- Перемещаем игрока в направлении цели
                humanoidRootPart.CFrame = humanoidRootPart.CFrame + direction * (moveSpeed * RunService.Heartbeat:Wait())
...
-- Функция для проверки объектов в папке Food
local function checkFoodObjects()
    -- Получаем текущее значение size из leaderstats
    local playerSize = sizeStat.Value
    
    for _, item in pairs(FoodFolder:GetChildren()) do
        local itemName = item.Name
        local itemNumber = tonumber(itemName)
        
        if itemNumber and itemNumber >= playerSize then
            print("Found item:", itemName)
            moveToPosition(player, item.Position)
        end
    end
end
...
-- Добавляем обработчик для изменения значения Size, чтобы проверять объекты при каждом изменении Size
sizeStat.Changed:Connect(checkFoodObjects)